...
async function CabanasPageContent({
  ...
  const properties = await fetchFilteredProperties({ ...filters, category: 'Cabaña' }, currentPage);

  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="border-b border-slate-100 pb-4">
        <h1 className="text-2xl font-black text-slate-800 tracking-tight">Cabañas</h1>
        <p className="text-xs text-slate-500 mt-1">Alojamientos rústicos e integrados con la naturaleza en la Sierra o junto al mar.</p>
      </div>
      ...
      <div className="flex items-center justify-between px-1 text-xs text-slate-500 font-semibold">
        ...
        <span><strong className="text-slate-800 font-bold">{properties.length}</strong> cabañas</span>
      </div>
      ...
      {properties.length > 0 ? (
        ...
        <div className="bg-white/80 border border-slate-100 rounded-3xl p-12 text-center max-w-md mx-auto space-y-3">
          <p className="text-slate-500 text-sm font-bold">No encontramos cabañas que coincidan con tu búsqueda.</p>
        </div>
      )}
    </div>
  );
...
export default async function Page({
  ...
  return (
    ...
    >
      <CabanasPageContent filters={filters} currentPage={currentPage} />
    </Suspense>
  );
}
