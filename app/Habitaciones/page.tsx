...
async function HabitacionesPageContent({
  ...
  const properties = await fetchFilteredProperties(query, currentPage, 'Habitación');
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="bg-white/80 border border-slate-100 p-4 rounded-2xl shadow-sm flex flex-col sm:flex-row gap-4 items-center justify-between">
        ...
        <div className="text-xs text-slate-500 font-semibold shrink-0">
          Encontradas: <span className="text-slate-800 font-bold">{properties.length}</span> habitaciones
        </div>
      </div>
...
export default async function Page({
  ...
  return (
    ...
    >
      <HabitacionesPageContent query={query} currentPage={currentPage} />
    </Suspense>
  );
}